...
const COLORS = ["#6C63FF", "#FF6584", "#43D9AD", "#FFB347", "#5BC0EB", "#FFC0CB", "#A8E6CF", "#888"];
...
export default function Analytics() {
  ...
  if (!data) return <div className="card"><p style={{color:"#888"}}>Loading analytics...</p></div>;
      ...
      <div className="analytics-row">
        <div className="card">
          ...
          {catData.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                <Pie data={catData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={75} label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`} labelLine={false} fontSize={11}>
          ...
          <div className="payment-list">
            {data.byPayment.length === 0 ? (
              ...
              <div key={i} className="payment-row">
                <div className="payment-dot" style={{ background: COLORS[i % COLORS.length] }} />
                <span className="payment-name">{p.payment_method.replace("_", " ")}</span>
                <span className="payment-val">{fmt(p.total)}</span>
                <span className="payment-pct">{total ? ((p.total / total) * 100).toFixed(0) : 0}%</span>
              </div>
            ))}
            <div className="payment-row total-row">
              <div className="payment-dot" style={{ background: "transparent" }} />
              <span className="payment-name" style={{ fontWeight: 700 }}>Total</span>
              <span className="payment-val" style={{ fontWeight: 700 }}>{fmt(total)}</span>
            </div>
          </div>
        </div>
      </div>
